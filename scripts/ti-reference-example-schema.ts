...
export type TiReferenceAnnotation = TiReferencePoint & {
  text: string;
  fontSize?: number;
};
...
export type TiReferenceTrace = {
  ...
  path?: string[];
  evidenceLabel: string;
  schematicRouteHints?: TiReferencePoint[];
};
...
export type TiGeneratedReferenceExample = TiReferenceExampleBase & {
  ...
  annotations?: TiReferenceAnnotation[];
};
...
export const validateTiReferenceExampleManifest = (
  ...
  for (const [index, rawExample] of value.examples.entries()) {
    ...
    if (rawExample.annotations !== undefined) {
      if (!Array.isArray(rawExample.annotations)) {
        throw new Error(`${location}.annotations must be an array`);
      }
      for (const [
        annotationIndex,
        annotation,
      ] of rawExample.annotations.entries()) {
        const annotationLocation = `${location}.annotations[${annotationIndex}]`;
        assertPoint(annotation, annotationLocation);
        assertRecord(annotation, annotationLocation);
        assertNonEmptyString(annotation.text, `${annotationLocation}.text`);
      }
    }
    ...
    for (const [traceIndex, trace] of rawExample.traces.entries()) {
      ...
      if (trace.schematicRouteHints !== undefined) {
        if (
          !Array.isArray(trace.schematicRouteHints) ||
          trace.schematicRouteHints.length === 0
        ) {
          throw new Error(
            `${traceLocation}.schematicRouteHints must be a non-empty array`,
          );
        }
        for (const [hintIndex, hint] of trace.schematicRouteHints.entries()) {
          assertPoint(
            hint,
            `${traceLocation}.schematicRouteHints[${hintIndex}]`,
          );
        }
      }
    }
...
export const renderTiReferenceExample = (
  ...
  const annotations = (example.annotations ?? []).map(
    (annotation) => `    <schematictext
      text=${json(annotation.text)}
      schX={${annotation.x}}
      schY={${annotation.y}}
      fontSize={${annotation.fontSize ?? 0.2}}
      anchor="center"
    />`,
  );
    ...
    <trace
      ${endpoints}${
        trace.schematicRouteHints
          ? `\n      schematicRouteHints={${JSON.stringify(trace.schematicRouteHints)}}`
          : ""
      }
    />`;
  })
